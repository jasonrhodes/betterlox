...
function SyncWatchData({
  ...
  const handleSyncClick = async (userId: number) => {
    ...
    try {
      const response = await callApi<ApiSuccessResponse & { syncedCount: number; } | ApiErrorResponse>(`/api/users/${userId}/entries/sync`, {
        method: 'POST'
      });
      if (response.data.success) {
        setSyncState("success");
        setSyncMessage(`Recent entries sync successfully completed. Found ${response.data.syncedCount} new entries.`);
      } else {
        setSyncState("failed");
